---
title: "Modeling Choices #01. Gaussian"
author: 
...
setwd("~/Treatment-effect-heterogeneity/Multisite Trials/docs/02_Modeling choices_01_Gaussian")
...
where $k = 1, ..., K$, $K$ is the number of second-stage units under analysis, $\sigma_{k}^2$ is the variance of the observed data, $y_{k}$, and $f(G)$ is the prior distribution of $G$. The site-specific parameters of interest, $\theta_{k} (k = 1, ..., K)$ comes from the population distribution, $G$. The observations, $y_{k} (k = 1, ..., K)$ comes from a Gaussian sampling distribution that depends on the $\theta_{k}$'s. The *Bayes deconvolution problem* is a general name for estimating $G$ given a random sample from the marginal density of $Y$ [@efron2016empirical]. 



## The @rubin1981estimation model^[See @meager2019understanding pp. 62-63.]

Now let us translate the sampling model into the @rubin1981estimation model on parallel randomized experiments. Consider $K$ study sites in which researchers perform the same interventions and measure the same outcomes. Each site, indexed by $k$, estimates a treatment effect $\tau_k$ averaged across individuals in the site. The sites don't report $\{\tau_k\}^{K}_{k=1}$: instead, they report $\{\hat{\tau}_k\}^{K}_{k=1}$. Some of the observed variation in $\{\hat{\tau}_k\}^{K}_{k=1}$ is sampling variation, yet there is likely to be some genuine variation in effects across settings, often defined as $\sigma_{\tau}^{2} = \text{var}(\tau_{k})$. 
...
The @rubin1981estimation model is fully parametric and a particular case of a more general class of models on $G$. The Gaussian distributional link between $\{\tau_k\}^{K}_{k=1}$ and $(\tau, \sigma_{\tau}^2)$ ensures that the model nests the analytic framework of classical meta-analysis, the results of which are recovered by setting $\sigma_{\tau}^{2} = 0$ [@gelman2013bayesian]. If $\sigma_{\tau}^{2} = \infty$, the @rubin1981estimation model returns the original estimates. 


The Gaussian structure is tractable and offers lower mean squared error in many cases. @mcculloch2011misspecifying has shown that Gaussian hierachical models generally deliver reliable inference on the mean $\tau$ and variance $\sigma_{\tau}^2$ even when the underlying true distribution is not Gaussian. 



# Generate simulated data from a Gaussian distribution 
...
GenerateGaussian <- function(n, rgt, rsr) {
  ...
  mn_tau <- 0; sigma_tau <- sqrt(1) 
  
  ### Generate data (Note the difference between theta and Y)
  data_obj <- list()
  data_obj$tau_k <- rnorm(n, mn_tau, sigma_tau)        # site means
  data_obj$tau_k_hat <- rnorm(n, data_obj$tau_k, sd) # within-site outcome (only one per site) 
  data_obj$se_k2 <- sigma2                  # site SDs
  return(data_obj)
}
...
First, we obtain simulates from the posterior of a Gaussian model using **Stan**. The **Stan** program for the @rubin1981estimation model is provided below:
...
model {
  sigma_tau ~ uniform(0, 100);     // vague prior
...
Note that the Rubin model assumes Gaussian distribution of effects across sites. The Rubin model performs univariate shrinkage so we will not need to specify a hyper-correlation prior. But we need to specify hyperpriors on the hyper-mean $\tau$ and hyper-SD $\sigma_{\tau}$:
...
fit_rubin<- stan(file = "rubin_model_01_Gaussian.stan",
                 data = stan_data,
                 iter = 1000, chains = 4)
```
...
Second, we obtain simulates from the posterior of a Gaussian model using the `hhsim` package. The **Stan** program above runs 4 chains with a burn-in period of 500 iterations followed by 500 additional iterations per chain. Since the `hhsim` package does not use separate chains, we set the number of  MCMC and burn-in iterations to 2,000, respectively, in order for the total iterations to be the same with those of the **Stan** program.    
...
### Estimate the Gaussian model using hhsim package
outp <- GaussianGaussian(Y, sigma2, nDraws)
...
From the results presented below, we can conclude that the **Stan** program and the `hhsim` package generates more or less similar posterior distributions for (1) hyperparameters ($\tau$ and $\sigma_{\tau}$), (2) empirical distribution function (EDF) estimates for $\tau_{k}$s, and (3) site-specific individual effect estimates $\tau_{k}$s.     
