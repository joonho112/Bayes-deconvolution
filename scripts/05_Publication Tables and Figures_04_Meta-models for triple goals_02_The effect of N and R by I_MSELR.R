...
###' 
###'  [Figure 05-1]. Interaction plots for Meta-model regression results 
###'  
###'   - MSELR (MSEL & ISEL are missing)
###'  
...
###' [Figure 05-01]. 
###' 
###' [Panel A] Effects of increasing N on the MSEL of rank (PM)
###' 
...
pA <- ggplot(data = df_plot_sub, 
  ...
  labs(title = "The effect of N on the MSEL of ranks (PM)", 
...
###' [Figure 05-01]. 
###' 
###' [Panel B] Effects of increasing N on the MSEL of rank (GR)
###' 
###' By shrinkage factor I
###' 
###'
...
pB <- ggplot(data = df_plot_sub, 
  ...
  theme(strip.background = element_rect(fill = "gray100")) + 
  theme(legend.position = "right", legend.direction = "vertical") + 
  labs(title = "The effect of N on the MSEL of ranks (GR)", 
...
###' [Figure 05-01]. 
###' 
###' [Panel C] Effects of R on the MSEL of ranks (PM)
###' 
###' By shrinkage factor I
###' 
###'
...
pC <- ggplot(data = df_plot_sub, 
  ...
  scale_y_continuous(labels = comma, 
                     limits = c(-150, 100), 
                     breaks = seq(-150, 100, by = 50)) + 
  scale_color_manual(values = color_palette[seq(unique(df_plot_sub$ovar1))]) +
  scale_shape_manual(values = shape_palette[seq(unique(df_plot_sub$ovar1))]) +
  theme_trend +
  theme(strip.background = element_rect(fill = "gray100")) + 
  theme(legend.position = "right", legend.direction = "vertical") + 
  labs(title = "The effect of R on the MSEL of ranks (PM)", 
       # subtitle = "Estimator: PM", 
       y = "Effect estimate", x = NULL)
...
###' [Figure 05-01]. 
###' 
###' [Panel D] Effects of R on the MSEL of ranks (GR)
###' 
###' By shrinkage factor I
###' 
###'
...
pD <- ggplot(data = df_plot_sub, 
  ...
  theme(legend.position = "right", legend.direction = "vertical") + 
  theme(strip.background = element_rect(fill = "gray100")) + 
  labs(title = "The effect of R on the MSEL of ranks (GR)", 
       # subtitle = "Estimator: GR", 
       y = "Effect estimate", x = NULL)
...
setwd(work_dir)
ggsave("figures/Figure05-01_Effects of N and R on MSELR_01_Mixed.pdf", 
       p_all, width = 8, height = 10)


### Only two combinations
p_two <- cowplot::plot_grid(pB, pD, ncol = 1, labels = "AUTO")

setwd(work_dir)
ggsave("figures/Figure05-01_Effects of N and R on MSELR_01_Mixed_Succinct.pdf", 
       p_two, width = 8, height = 5)
