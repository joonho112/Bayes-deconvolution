...
###'       
###'       3-1. Mean squared error loss (MSEL) for 
...
### Define labels
lab_elem0 <- c("MSEL_for_individual_effects")
title_vec <- "Mean Squared Error Loss (MSEL) for the individual site-specific effects"
...
for (i in seq(nrow(tbl_loop))){
  ...
  ### Extract parameters to filter the dataset
  elem0 <- c("SSEL")
...
for (i in seq(nrow(tbl_loop))){
  ...
  ### Extract parameters to filter dataframe
  elem0 <- c("SSEL")
  elem1 <- as.character(tbl_loop[i, 1])
  elem2 <- as.character(tbl_loop[i, 2])
