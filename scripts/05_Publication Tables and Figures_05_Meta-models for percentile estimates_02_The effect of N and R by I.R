...
###' 
###'  [Figure 08]. Interaction plots for Meta-model regression results 
###'  
###'   - Raw biases for percentile estimates
###'   - The effect of N by I levels, DGM, and anl
###'           
###' Data: Simulated data
###' 
###' Data: 2020-04-30
...
###' [Figure 08]. 
###' 
###' [Panel A] Effects of increasing N on the bias of percentile estimates
###' 
...
df_grid <- expand.grid(vec_tail_p, vec_anl, vec_truth) %>%
  data.frame() %>%
  dplyr::select(Var3, Var2, Var1) %>%
  set_names(c("truth", "anl", "tail_p")) 
...
for (i in seq(nrow(df_grid))){
  ...
  list_df_plot[[i]] <- lincom[[2]] %>%
    mutate(elem1 = elem_truth, elem2 = elem_anl, elem3 = elem_tail_p) %>%
...
### Plot A: The effect of N on the bias of 50 percentile (GR)
df_plot_sub <- df_plot %>% 
  filter(elem3 == 50)
...
pA <- ggplot(data = df_plot_sub, 
  ...
  labs(title = "The effect of N on the bias of 50 percentile estimate (GR)", 
...
pB <- ggplot(data = df_plot_sub, 
  ...
  theme(strip.background = element_rect(fill = "gray100")) + 
  theme(legend.position = "right", legend.direction = "vertical") +  
  labs(title = "The effect of N on the bias of 90 percentile estimate (GR)", 
       # subtitle = "Estimator: PM", 
       y = "Effect estimate", x = NULL)
...
setwd(work_dir)
ggsave("figures/Figure08_Effects of N on percentile estimates.pdf", 
